...
//
// IMPORTANTE: o pedido é sempre relido do Firestore pelo ID recebido —
// nunca confiamos nos dados de "order" enviados no corpo da requisição.
// Sem isso, qualquer chamada direta (curl) poderia forjar um pedido falso
// e injetar HTML arbitrário no e-mail de notificação.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    order = { ...snap.data(), id: snap.id };
  } catch (e) {
    console.error("notify: falha ao buscar pedido", e.message);
    return res.status(500).json({ error: "Erro ao buscar pedido" });
  }

  // ─── Idempotência: evita reenvio/spam de notificações ──────────────
  // Esta rota não exige autenticação (o checkout público precisa poder
  // chamá-la), então sem essa trava qualquer pessoa que soubesse o ID de
  // um pedido poderia chamar /api/notify repetidamente e gerar custo/spam
  // nas integrações (CallMeBot, Resend, FCM) sem limite. Marca o pedido
  // como notificado dentro de uma transação (evita corrida entre duas
  // chamadas simultâneas) e recusa reenvio.
  try {
    const db = getAdminDb();
    const alreadyNotified = await db.runTransaction(async (tx) => {
      const snap = await tx.get(orderRef);
      if (snap.data()?.notifiedAt) return true;
      tx.update(orderRef, { notifiedAt: new Date().toISOString() });
      return false;
    });
    if (alreadyNotified) {
      return res.status(200).json({ results: { skipped: "already_notified" }, errors: [] });
    }
  } catch (e) {
    console.error("notify: falha ao marcar notifiedAt", e.message);
        ...
        <div style="font-family:sans-serif;max-width:520px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #eee;">
          ...
          <div style="padding:24px;">
            ...
            <table style="width:100%;border-collapse:collapse;">
              ${(order.itens || []).map(i => `
              <tr>
                <td style="padding:8px 0;border-bottom:1px solid #eee;font-size:13px;color:#333;">${escHtml(i.qty)}× ${escHtml(i.nome)}</td>
            ...
            <div style="margin-top:18px;background:#FDF6E8;border-radius:8px;padding:12px 14px;font-size:12px;color:#666;">
              <div><strong>Cliente:</strong> ${escHtml(order.cliente?.nome) || "—"}</div>
              <div><strong>WhatsApp:</strong> ${escHtml(order.cliente?.wpp) || "—"}</div>
              <div><strong>Pagamento:</strong> ${escHtml(pagamento)}</div>
              <div><strong>Retirada:</strong> ${escHtml(dataStr)}</div>
            </div>
          </div>
        </div>`;
